...
public class RatingsCRUD {
    ...
    public static int addRating(Ratings rating) {
        String query = "INSERT INTO ratings (userId, tripRating, vehicleRating, driverRating, comment) VALUES (?, ?, ?, ?, ?)";
        try (Connection conn = ConnectionHelper.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
            stmt.setInt(1, rating.getUserId());
            stmt.setBigDecimal(2, rating.getTripRating());
            stmt.setBigDecimal(3, rating.getVehicleRating());
            stmt.setBigDecimal(4, rating.getDriverRating());
